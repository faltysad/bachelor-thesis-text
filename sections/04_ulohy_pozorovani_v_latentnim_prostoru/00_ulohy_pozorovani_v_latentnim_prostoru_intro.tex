...
Další aplikací VAE je konceptuální komprese \cite{Gregor2016}. Tato problémová oblast úzce souvisí s informační teorií a interpretací VAE z pohledu informační teorie (viz \autoref{sec:vae_information_theory_interpretation}).
...
Jelikož značný počet bitů každého obrázku slouží jako nositel těchto \emph{detailnějších} atributů, lze jejich zanedbání využít za účelem ztrátové komprese pomocí VAE. A na tomto principu tedy obecně funguje konceptuální (ztrátová) komprese pomocí VAE.

Tento generativní model založený na rozšíření VAE (DRAW, viz \autoref{sec:draw}) nabízí \textbf{větší kvalitu než kompresní algoritmy JPEG a JPEG 2000} ve všech úrovních detekovatelné korupce výsledného obrázku (pro detailnější srovnání, viz \cite{Gregor2016}).
