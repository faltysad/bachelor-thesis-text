...
\begin{figure}[H]
    ...
    \node[trapezium,
        ...
        minimum width=5cm,
        rotate=270] (encoder) at (1, 0) {Encoder};
    ...
    \node[trapezium,
        ...
        minimum height=2.5cm,
        minimum width=5cm,
        rotate=90] (decoder) at (6, 0) {Decoder};
    ...
    \end{tikzpicture}
    \caption{Obecné schéma modulů Autoenkodéru}
...
\subsection{Autoenkodér s neúplnou skrytou vrstvou // Undercomplete Autoencoder}
Pouhé kopírování vstupů na výstupní vrstvu se nemusí jevit jako zvlášť užitečné.
Nicméně při trénování Autonekodéru se snažíme aby \emph{h} vypozorovala užitečné vlastnosti vstupních dat.

Jedním způsobem, jak docílit \textbf{kompresované reprezentace znalostí} je \textbf{omezit dimenzi \emph{h} tak, aby byla nižśí než dimenze \emph{x}}.
Takovou skrytou vrstvu \emph{h} nazýváme \textbf{bottleneck}.

Autoenkodér, jehož dimenze kódu (\emph{h}) je nižší, než dimenze vstupu nazýváme \textbf{Autoenkodér s neúplnou skrytou vrstvou} (\emph{undercomplete autoencoder}).
Učení kompresované reprezentace \emph{nutí} Autoenkodér zachytit pouze ty stěžejní vlastnosti trénovacích dat.
